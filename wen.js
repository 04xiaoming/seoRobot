...
var url = 'http://www.huangjinzhijia.com/lcxzs/2_8k/';
var bigclass = '月薪2-8K理财';
var arcList = [];
charset(agent); //
function filterHtml(html) {
    ...
    aPost.each(function() {
        ...
        agent.get(url).charset('gbk').end(function(err, res) {
            ...
            content = content.replace(/<br>{3,6}/g, "<br>");
...
function nextPage(html) {
    var $ = cheerio.load(html);
    var nextUrl = $(".pgs a:last-child").attr('href');
    console.log(nextUrl);
    ...
    if (curPage > nextPage) //控制结束写入
    {
        ...
        //生成json文件
        fs.writeFile('./index555.json', JSON.stringify(arcList), {
            'flag': 'a'
        }, function(err) {
            if (err) {
                throw err;
